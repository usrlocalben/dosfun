#include "app_player_adapter.hpp"

#include "kb_tinymod.hpp"
#ifdef SHOW_TIMING
#include "vga_reg.hpp"
#endif

#include <algorithm>
#include <cstdint>
#include <limits>

using std::int8_t;
using std::uint8_t;
using std::int16_t;
using std::uint16_t;

namespace rqdq {
namespace {

int FP16toS16_fast(int x) {
	return x >> 1; }

int FP16toS16_slow(int x) {
	x = std::min(std::max(x, -0xffff), 0xffff);  // clamp
	return x >> 1; }

}
namespace app {

void PlayerAdapter::BlasterJmp(void* out, int fmt, int numChannels, int numSamples, void* self) {
	static_cast<PlayerAdapter*>(self)->BlasterProc(out, fmt, numChannels, numSamples); }


inline void PlayerAdapter::BlasterProc(void* out_, int fmt, int numChannels, int numSamples) {
#ifdef SHOW_TIMING
//vga::Color(255, { 0x20, 0x3f, 0x10 });
#endif
	if (rw_.Size() >= numSamples) {
		if (fmt == 2) {
			// 16-bit signed PCM
			int16_t* out = static_cast<int16_t*>(out_);
			for (int i=0; i<numSamples; i++) {
				int l, r;
				PopFront(l, r);
				if (numChannels == 2) {
					out[i*2+0] = std::clamp(l>>1, -32768, 32767);
					out[i*2+1] = std::clamp(r>>1, -32768, 32767); }
				else {
					out[i] = std::clamp((l+r)>>2, -32768, 32767); }}}
		else {
			// 8-bit unsigned PCM
			uint8_t* out = static_cast<uint8_t*>(out_);
			for (int i=0; i<numSamples; i++) {
				int l, r;
				PopFront(l, r);
				if (numChannels == 2) {
					out[i*2+0] = std::clamp((l>>9)+128, 0, 255);
					out[i*2+1] = std::clamp((r>>9)+128, 0, 255); }
				else {
					out[i] = std::clamp((l+r>>10)+128, 0, 255); }}}}
#ifdef SHOW_TIMING
//vga::Color(255, { 0, 0, 0 });
#endif
	}


void PlayerAdapter::Refill() {
#ifdef SHOW_TIMING
vga::Color(255, { 0x30, 0x20, 0x10 });
#endif
	// int numSamples = std::min(rw_.Available(), 128U);
	int numSamples = rw_.Available();
	if (numSamples > 0) {
		player_.Render(pbuf_, pbuf_+4096, numSamples);
		for (int i=0; i<numSamples; i++) {
			int l = pbuf_[i];
			int r = pbuf_[i+4096];
			PushBack(l, r); }}
#ifdef SHOW_TIMING
vga::Color(255, { 0, 0, 0 });
#endif
	}


}  // namespace app
}  // namespace rqdq
